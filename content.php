	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<?php if ( is_sticky() && is_home() && ! is_paged() ) { ?>
			<div class="featured-post">
				<?php esc_html_e( 'Featured post', 'sitepoint-base' ); ?>
			</div>
		<?php } ?>
		<header class="entry-header">
			...
			<?php if ( has_post_thumbnail() && !is_search() ) {
				if ( is_single() ) {
					...
					<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to ', 'sitepoint-base' ) . '%s', the_title_attribute( 'echo=0' ) ) ); ?>">
		...
		<?php }
			...
			<div class="entry-content">
				<?php if ( has_excerpt() && !is_single() ) {
					the_excerpt(); ?>
					<p><a class="more-link" href="<?php the_permalink(); ?>"><?php echo wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'sitepoint-base' ), array( 'span' => array(
						'class' => array() ) ) ) ?></a></p>
				<?php }
				else {
					the_content( wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'sitepoint-base' ), array( 'span' => array(
						'class' => array() ) ) )
						);
				}
				 ?>
